...
//    vdp_enable_layers(SCROLL0 | SPRITES);
    vdp_enable_layers(SCROLL0 | AFFINE | SPRITES);
    // AFFINE
    ...
    for (uint16_t y = 0; y < affine_ball_dimension; y++) {
        ...
        vdp_seek_vram(y * affine_map_dimension * 2);
    ...
    // Affine parameters (to move to some function with per frame updates_

    // (confirm these)

    vdp_set_affine_matrix(0x40, 0, 0, 0x40);
    vdp_set_affine_pretranslate(0, 0);
