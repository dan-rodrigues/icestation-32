...
verilator -cc -v config.vlt --trace -O3 --compiler clang -o ${SIM_NAME} ${RTL_INCLUDE} --assert -Wall -Wno-fatal -Wno-WIDTH --top-module "${TOP_MODULE}" "${TOP_MODULE}".v "${ICE40_CELLS_SIM}" -CFLAGS "${CFLAGS}" -LDFLAGS "${LDFLAGS}" --exe "${CXX_SOURCES}"
